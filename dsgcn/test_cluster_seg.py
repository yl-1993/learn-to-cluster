...
def test_cluster_seg(model, cfg, logger):
    assert os.path.isfile(cfg.pred_iou_score)
    ...
    pred_scores = d['data']
    meta = d['meta'].item()
    assert dataset.inst_num == meta['tot_inst_num'], '{} vs {}'.format(
        dataset.inst_num, meta['tot_inst_num'])
    ...
    # de-overlap (wo gcn-s)
    pred_labels_wo_seg = deoverlap(pred_scores, proposals, dataset.inst_num,
